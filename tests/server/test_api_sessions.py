...
async def test_compile_open_task_queue_claim_complete_ack_same_account(
    client: httpx.AsyncClient,
    service,
    monkeypatch,
):
    _queue_service, queues = _install_fake_open_task_queue(monkeypatch, service)
    alice_headers = {"X-OpenViking-Account": "queue-acct", "X-OpenViking-User": "alice"}
    other_headers = {"X-OpenViking-Account": "other-acct", "X-OpenViking-User": "eve"}
    ...
    assert claimed["claimed_by_user_id"] == "alice"
    ...
    wrong_lease = await client.patch(
        f"/api/v1/task-queues/compile/tasks/{task_id}",
        json={"lease_id": "lease_wrong", "progress": 0.6},
        headers=alice_headers,
    )
    ...
    assert get_resp.status_code == 200
    visible = get_resp.json()["result"]
    assert visible["status"] == "completed"
    assert visible["ack_by"] == "alice"
    assert visible["result"]["page_count"] == 1
    assert "lease_id" not in visible
    assert "queue_name" not in visible
    assert "queue_message_id" not in visible
...
async def test_compile_open_task_queue_recovered_expired_lease_can_be_claimed_again(
    ...
    queue = queues["OpenCompileTask"]
    first_claim = await queue_service.claim_compile_task(account_id="lease-acct", user_id="alice")
    ...
    second_claim = await queue_service.claim_compile_task(account_id="lease-acct", user_id="bob")

    assert second_claim is not None
    assert second_claim.task_id == created.task_id
    assert second_claim.attempt == 2
    assert second_claim.claimed_by_user_id == "bob"
