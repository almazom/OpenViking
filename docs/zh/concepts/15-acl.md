...
规则；父目录已有 ACL 时，创建者获得直接 `manager`，并继承父目录权限。

开启后，新建共享文件或目录即使位于无 ACL 的父目录下，创建者也会在首条 context
记录上获得直接 `manager`；父目录权限仍作为继承 ACL 合并。已有内容不会迁移或
改权，重新关闭也只影响后续创建。`add-resource` 只把本次生成的根目录（`no_split`
时为根文件）作为创建节点：根节点获得创建者直接 `manager`，内部节点只继承，
...
直接 `manager` 完成权限 bootstrap。其他情况下，首次设置 ACL 只能由共享区隐式
...
`auto_protect_new_content` 时，新创建节点为创建者生成直接 `manager`，并从父节点
