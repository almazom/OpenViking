...
| `commit` | `editor` (WRITE); directories recursively check every current descendant and fail as a whole if any node is denied |
| `restore` overwriting a file | `editor` (WRITE) on the file |
| `restore` creating a file | `editor` (WRITE) on the parent directory |
| `restore` deleting a file | `manager` (MANAGE) on the file |
...
Existing nodes keep their current ACL after restore. Newly restored nodes inherit the current parent ACL; the user who runs `restore` is not added as a manager. The background vector rebuild is system work for an already-authorized operation, so parent ACLs do not block it again.
